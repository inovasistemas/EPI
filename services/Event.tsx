...
export async function getEventsByCollaborator({
	loading,
	q,
	sortField,
	sortOrder,
	...
	try {
		...
		const response = await axios.get(
			`${process.env.NEXT_PUBLIC_API_HOST}/collaborator/${collaborator}/events`,
			{
				params,
				withCredentials: true,
			},
...
export async function getEvent({
	...
	try {
		...
		const response = await axios.get(
			`${process.env.NEXT_PUBLIC_API_HOST}/events/${id}`,
			{
				withCredentials: true,
			},
		);
