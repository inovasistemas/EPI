...
import {
	createRoutineEquipment,
	deleteRoutineEquipment,
	getRoutine,
	updateRoutine,
	updateRoutineEquipment,
...
const OperatorDetails: FC = () => {
	...
	const selectedCollaborators = useMemo(() => {
		if (!formData.collaborators?.length || !collaboratorsData?.length)
			return [];
	...
	const fetchCollaborators = async () => {
		const response = await getCollaborators({
			loading: setLoadingCollaborators,
		});
	...
	const handleChangeMulti = <T extends keyof formData>(
		name: T,
		value: formData[T],
	) => {
		setFormData((prev) => ({
	...
	const handleAddEquipment = async (
		...
		const alreadyExists = formData.equipments.some(
			(item) => item.equipment === uuid,
		);
	...
	const handleUpdateEquipmentQuantity = async (
		uuid: string,
		newQuantity: number,
	) => {
		if (!RoutineId) return;
	...
	const handleUpdateRoutine = async () => {
		...
		if (response) {
			if (response.status === 200) {
				...
				toast.custom(() => (
					<ToastSuccess text="Rotina atualizada com sucesso" />
				));
			} else if (response.status === 403) {
				toast.custom(() => (
					<ToastError text="Você não possui permissão para esta ação" />
										...
										<div className="flex flex-col gap-4 bg-[--backgroundSecondary] p-4 rounded-2xl">
											<div className="flex flex-col gap-1 pb-1">
												...
												<span className="opacity-70 text-[--textSecondary] text-sm">
													Escolha se a rotina será aplicado a um colaborador,
													setor ou ambos.
												</span>
											</div>
											<SelectCollaborators
									...
									<div className="flex flex-col gap-3 col-span-2">
										...
										<div className="flex flex-col gap-4 bg-[--backgroundSecondary] p-4 rounded-2xl">
											<div className="flex flex-col gap-1 pb-1">
												...
												<span className="opacity-70 text-[--textSecondary] text-sm">
													Escolha a frequência com que esta rotina deve se
													repetir.
												</span>
											</div>
											<div className="gap-3 grid grid-cols-2 w-full">
												<SearchSelect
													...
													onChange={(value: string) =>
														handleChange("cycle_auantity", value)
													}
													background="bg-[--backgroundPrimary]"
												/>
										...
										{equipmentsData &&
											equipmentsData.map((equipment, i) => (
												<Reorder.Item
													value={equipment}
													key={equipment.uuid}
													className="gap-3 grid grid-cols-2 bg-[--tableRow] rounded-xl font-normal text-[--textSecondary] text-sm capitalize"
												>
													...
													<div className="flex justify-end items-center gap-3 col-span-1 p-4 lowercase">
														...
														<button
															type="button"
															onClick={() =>
																handleDeleteEquipment(equipment.uuid)
															}
