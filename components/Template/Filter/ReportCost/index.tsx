...
type FilterReportCostProps = {
  ...
  saveAction: () => void
  action: (name: string, value: dayjs.Dayjs) => void
}

export function FilterReportCost({ start, end, action, saveAction }: FilterReportCostProps) {
  return (
    <div className='flex flex-col gap-8 w-full'>
      ...
      <div className='flex flex-col gap-6 divide-y divide-[--outlinePrimary] w-full'>
        <div className='items-center grid grid-cols-2 w-full select-none'>
          ...
          <div>
            <RangeDateInput start={start} end={end} onChange={action} />
          </div>
        </div>
      </div>
